...
async function detectTable(db) {
  ...
  for (const t of tables) {
    ...
    if (names.has('subclass') && (names.has('class') || names.has('class_name'))) {
      return t.name;
    }
  }

  throw new Error('No table found with subclass + class/class_name columns.');
}
...
async function main() {
  ...
  try {
    ...
    const rows = await all(db, `SELECT * FROM ${table}`);

    const normalized = rows.map(r => ({
      ...r,
      class: r.class ?? r.class_name ?? null
    }));

    fs.writeFileSync(OUT_PATH, JSON.stringify(normalized, null, 2), 'utf8');
    console.log(`Wrote ${OUT_PATH} (${normalized.length} rows) from table "${table}"`);
  } finally {
    db.close();
  }
}
